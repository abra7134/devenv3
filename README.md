...
**php5.6-fpm**, **php7.1-fpm**, **php7.2-fpm** и **php7.4-fpm** с необходимыми преднастройками
(почти идентично повторяющими настройки боевых серверов). Окружение разработки доступно под локальным доменом:
...
В Окружении разработки доступны **5.6**, **7.1**, **7.2** и **7.4** версии **PHP**-интерпретатора,
...
(а именно пустые файлы `.profile_php7.1`, `.profile_php7.2` или `.profile_php7.4` (обратите внимание на точку в начале).
...
В большинстве случаев на боевых серверах установлены пограничные балансировщики (в том числе и **SSL**-терминаторы),
которые запрос из Интернета траслируют уже во внутреннюю сеть приложения (кластера). Тем самым **PHP**-переменная
`REMOTE_ADDR` будет содержать в себе не **IP**-адрес реального клиента, который послал запрос, а **IP**-адрес
конкретного балансировщика. Оригинальный же **IP**-адрес будет записан балансировщиком в **HTTP**-заголовки
`X-Forwarded-For` и `X-Real-Ip`. А в заголовки `X-Forwarded-Proto` и `X-Real-Ip` запишутся, соответственно,
изначальный протокол и оригинальный TCP-порт клиента.

В Окружении разработки балансировщик отсутствует.
Но для удобства разработки эти заголовки устанавливаются в принудительном порядке **HTTP**-сервером.

`X-Real-Ip` будет содержать в себе IP-адрес клиента (в окружении разработки будет совпадать с **PHP**-переменной `REMOTE_ADDR`)
`X-Real-Port` будет содержать TCP-порт клиента, соответственно.
`X-Forwarded-For` и `X-Forwarded-Proto` - цепочку IP-адресов и изначальный протокол соответственно.

Например:
```
X-Real-Ip: 192.168.1.23
X-Real-Port: 56723
X-Forwarded-For: 192.168.0.10, 192.168.1.23
...
api-1         http://api-1.localhost/      == api-1/             index.php            7.4         -
