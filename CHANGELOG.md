...
- **docker-compose.yml**: В качестве временного решения отключена изоляция сетевого стека контейнеров
  (параметр `network_mode: host`), что позволяет прямое обращение `PHP`-приложений к `PHP`-приложениям
  через стандартный адрес вида: `http://*.localhost`
