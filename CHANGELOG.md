...
### v0.4.4 ()
- **docker-compose.yml**: Исправлен проброс build-переменных при использовании YAML-overrides

### v0.4.3 (2022/07/06)
- Большинство контейнеров теперь используют Debian **10** в качестве базового слоя

### v0.4.2 (2022/06/28)

- **devenv3.bash**: В команду `build` добавлена проверка существования `.env` файла
- **php-fpm-72**, **php-fpm-74**: Восстановлена работа трассировки (через `xdebug`-расширение)
- **php-fpm-**: Добавлен пакет `imagemagick` для работы с изображениями
